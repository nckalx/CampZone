...
router.post("/register", function(req, res) {
    ...
    User.register(newUser,req.body.password, function(err, user){
        if (err) {
              req.flash("error", "An email with that address is already in use.");
...
router.post("/login", passport.authenticate("local", 
    {
        ...
        failureFlash: true,
        successFlash: "Welcome back to Campzone!"
    }), function(req, res) {
});
  ...
  async.waterfall([
    ...
    function(token, user, done) {
      var smtpTransport = nodemailer.createTransport({
        service: 'Gmail', 
        auth: {
          user: 'learningtocodenick@gmail.com',
          pass: process.env.GMAILPW
        }
      });
      var mailOptions = {
        to: user.email,
        from: 'learningtocodenick@gmail.com',
...
// DESTROY USER PROFILE AND ASSOCIATED CAMPGROUNDS ROUTE
